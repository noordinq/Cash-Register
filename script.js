...
function formatResults(status, change) {
    ...
    change.map(
        money => (displayChange.innerHTML += `<p>${money[0]}: $${money[1]}</p>`)
    );
    return;
}
...
const checkRegister = () => {
  ...
  let dueChange = Number(cash.value) - price;
  let result = {status: 'OPEN', change: []}
  let reverseCid = [...cid].reverse();
  let denominations = [100, 20, 10, 5, 1, 0.25, 0.1, 0.05, 0.01];
  let totalCid = parseFloat(cid.map(total => total[1])
                            .reduce((prev, curr) => prev + curr)
                            .toFixed(2)); 

  if (totalCid < dueChange) {
    return displayChange.innerHTML = '<p>Status: INSUFFICIENT_FUNDS</p>';
  }
  ...
  for(let i = 0; i <= reverseCid.length; i++) {
    if(dueChange > denominations[i] && dueChange > 0) {
      let count = 0;
      let total = reverseCid[i][1];
      while (total > 0 && dueChange >= denominations[i]) {
        total -= denominations[i];
        dueChange = parseFloat((dueChange -= denominations[i]).toFixed(2));
        count++;
      }
      if(count > 0){
...
purchase.addEventListener("click", checkRegister); */
